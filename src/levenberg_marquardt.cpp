...
struct SimpleLineFunctorNumericalDiff
    : Eigen::NumericalDiff<SimpleLineFunctor> {};

Point2DVector GeneratePoints(const unsigned int numberOfPoints) {
  Point2DVector points;
  // Model y = 2*x + 5 with some noise (meaning that the resulting minimization
  // should be about (2,5)
...
void quadraticPointsGenerator(std::vector<double> &x_values,
                              std::vector<double> &y_values, double a = -1,
                              double b = 0.6, double c = 2,
                              unsigned int numberOfPoints = 50) {
...
void testQuadraticFunctor() {
  ...
  // The first column has the input values, x. The second column is the f(x)
  // values.
  ...
  // 'parameters' is vector of length 'n' containing the initial values for the
  // parameters. The parameters 'x' are also referred to as the 'inputs' in the
  // context of LM optimization. The LM optimization inputs should not be
  // confused with the x input values.
...
///////////////////////// Levenberg Marquardt Example (3) f(x,y) = (x + 2*y
///-7)^2 + (2*x + y - 5)^2 ///////////////////////////////
...
struct BoothFunctor : Functor<double> {
  ...
  int operator()(const Eigen::VectorXd &z, Eigen::VectorXd &fvec) const {
     ...
     * Important: LevenbergMarquardt is designed to work with objective
     * functions that are a sum of squared terms. The algorithm takes this into
     * account: do not do it yourself. In other words: objFun = sum(fvec(i)^2)
     */
    fvec(0) = x + 2 * y - 7;
    fvec(1) = 2 * x + y - 5;
    return 0;
...
void testBoothFunctor() {
  std::cout
      << "Testing the f(x,y) = (x + 2*y -7)^2 + (2*x + y - 5)^2 function..."
      << std::endl;
  ...
  Eigen::LevenbergMarquardt<Eigen::NumericalDiff<BoothFunctor>, double> lm(
      numDiff);
...
void testHimmelblauFunctor() {
  std::cout << "Testing f(x,y) = (x^2 + y - 11)^2 + (x + y^2 - 7)^2 function..."
            << std::endl;
  ...
  Eigen::LevenbergMarquardt<Eigen::NumericalDiff<HimmelblauFunctor>, double> lm(
      numDiff);
  lm.parameters.maxfev = 1000;
  lm.parameters.xtol = 1.0e-10;
  std::cout << "max fun eval: " << lm.parameters.maxfev << std::endl;
