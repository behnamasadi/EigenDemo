...
template <typename scalar> struct product_functor {
  product_functor(scalar a, scalar b) : m_a(a), m_b(b) {
    std::cout << "Type: " << name_of_type<scalar>()
...
struct sum_of_ints_functor {
  sum_of_ints_functor(int a, int b) : m_a(a), m_b(b) {
    std::cout << "Type: int. Computing the sum of the two ints " << a << " and "
              << b << ".";
  }
