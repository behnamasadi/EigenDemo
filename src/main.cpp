...
void MatrixPowersPolynomials() {
  ...
  // rectangular diagonal  matrix is an n × d matrix in which each entry (i, j)
  // has a non-zero value if and  only if i = j. diagonal matrix is a matrix in
  // which the entries outside the main diagonal are all zero;

  // A block diagonal matrix contains square blocks B 1…B r of (possibly)
  // nonzero entries along the diagonal. All other entries are zero. Although
  // each block is square, they need not be of the same size.

  // Upper and Lower Triangular Matrix) A square matrix is an upper triangular
  // matrix if all entries (i, j) below its main diagonal (i.e., satisfying
  // i > j) are zeros.
  ...
  // Strictly Triangular Matrix) A matrix is said to be strictlytriangular if it
  // is triangularandall its diagonal elements are zeros.

  // The zeroth power of a matrix is defined to be the identity matrix
  // When a matrix satisfies A^k = 0 for some integer k, it is referred to as
  // nilpotent. all strictly triangular matrices (triangular with zero main
  // diagonal)of size d × d satisfy A^d = 0. product of two upper triangular is
  // a triangular matrix

  // polynomial function f(A) of a square matrix in  much the same way as one
  // computes polynomials of scalars. f(x) = 3x^2 + 5x + 2 ->
  // f(A) = 3A^2 + 5A + 2 Two    polynomials f(A) and g(A) of the same matrix A
  // will always commute f(A)g(A)=g(A)f(A)
  ...
  // A*A^T=A^T*A=I all column columns/ rows are perpendicular
  ...
  Therefore, the n × d matrix A is occasionally represented in terms of its
  ordered set of ndimensional columns A nxd= [a1 a2 ... ad]

   low-rank update
   Linear regression least-squares classification, support-vector machines, and
  logistic regression Matrix factorization is an alternative term for matrix
  decomposition recommender systems

  Regression
  the only difference from classification is that the array contains numerical
  values (rather than categorical ones) The dependent variable is also referred
  to as a response variable, target variable, or regressand in the case of
  regression The independent variables are also referred to as regressors.more
  than two classes like {Red, Green, Blue} cannot be ordered, and are therefore
