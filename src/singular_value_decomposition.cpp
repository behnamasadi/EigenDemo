...
void SVD_Example() {
  ...
  */
  ...
  std::cout << "Size of original matrix:" << A.rows() << "," << A.cols()
            << std::endl;

  std::cout << "Size of U matrix:" << svd.matrixU().rows() << ","
            << svd.matrixU().cols() << std::endl;

  std::cout << "Size of Σ matrix:" << svd.singularValues().rows() << ","
            << svd.singularValues().cols() << std::endl;

  std::cout << "Size of V matrix:" << svd.matrixV().rows() << ","
            << svd.matrixV().cols() << std::endl;
  ...
  std::cout << "This should be zero vector (solution of the problem A*V.col( "
               "V.cols()-1))"
            << std::endl;
  ...
  Eigen::MatrixXd C;
  C.setRandom(27, 18);
  Eigen::JacobiSVD<Eigen::MatrixXd> svd_thin(C, Eigen::ComputeThinU |
                                                    Eigen::ComputeThinV);

  std::cout << "Size of original matrix:" << C.rows() << "," << C.cols()
            << std::endl;

  std::cout << "Size of U matrix:" << svd_thin.matrixU().rows() << ","
            << svd_thin.matrixU().cols() << std::endl;

  std::cout << "Size of Σ matrix:" << svd_thin.singularValues().rows() << ","
            << svd_thin.singularValues().cols() << std::endl;

  std::cout << "Size of V matrix:" << svd_thin.matrixV().rows() << ","
            << svd_thin.matrixV().cols() << std::endl;

  Eigen::MatrixXd C_reconstructed = svd_thin.matrixU() *
                                    svd_thin.singularValues().asDiagonal() *
                                    svd_thin.matrixV().transpose();

  std::cout << "diff:\n" << (C - C_reconstructed).array().abs().sum() << "\n";

  Eigen::MatrixXd pinv_C = svd_thin.matrixV() *
                           svd_thin.singularValues().asDiagonal() *
                           svd_thin.matrixU().transpose();
  ...
  std::cout << "Size of pinv_C matrix:" << pinv_C.rows() << "," << pinv_C.cols()
            << std::endl;
