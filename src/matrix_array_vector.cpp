...
void coefficientWiseOperations() {
  ...
  std::cout << "The minimum element is: " << my_matrix.minCoeff(&i, &j)
            << " and its indices are:" << i << "," << j << std::endl;

  std::cout << "The maximum element is: " << my_matrix.maxCoeff(&i, &j)
            << " and its indices are:" << i << "," << j << std::endl;

  std::cout << "The multipication of all elements: " << my_matrix.prod()
            << std::endl;
  ...
  std::cout << "The means of columns: " << my_matrix.colwise().mean()
            << std::endl;
  std::cout << "The max of each columns: " << my_matrix.rowwise().maxCoeff()
            << std::endl;
  std::cout << "Norm 2 of the matrix is: " << my_matrix.lpNorm<2>()
            << std::endl;
  std::cout << "Norm infinty of the matrix is: "
            << my_matrix.lpNorm<Eigen::Infinity>() << std::endl;
  std::cout << "If all elemnts are positive: " << (my_matrix.array() > 0).all()
            << std::endl;
  std::cout << "If any element is greater than 2: "
            << (my_matrix.array() > 2).any() << std::endl;
  std::cout << "Counting the number of elements greater than 1"
            << (my_matrix.array() > 1).count() << std::endl;
