...
struct SubstrateConcentrationFunctor : Functor<double> {
  ...
  SubstrateConcentrationFunctor(Eigen::MatrixXd points)
      : Functor<double>(points.cols(), points.rows()) {
    this->Points = points;
  }
  ...
  int inputs() const {
    return 2;
  } // There are two parameters of the model, beta1, beta2
  int values() const {
...
void SubstrateConcentrationLeastSquare() {
  ...
  */
  ...
  Eigen::NumericalDiff<SubstrateConcentrationFunctor,
  ...
  std::cout << "jacobian of matrix at " << beta(0) << "," << beta(1)
            << " is:\n " << J << std::endl;
...
int main() { SubstrateConcentrationLeastSquare(); }
