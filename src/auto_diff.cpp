...
template <typename T> T MultiVarFunction(const T &x, const T &y) {
  T z = x * cos(y);
  return z;
}
...
struct VectorFunction {
  ...
  void operator()(const Eigen::Matrix<Scalar, NX, 1> &x,
                  Eigen::Matrix<Scalar, NY, 1> *y) const {
...
void JacobianDerivativesExample() {
  ...
  std::cout << "jacobian of matrix at " << x(0, 0) << "," << x(1, 0)
            << " is:\n " << fjac << std::endl;
}
